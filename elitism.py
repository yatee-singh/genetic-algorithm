    ...
    for size in sizes:
        ...
        #   Initial state: defining a population
            ...
            while(generations<=total_generations):
                ...
                sorted_population= sorted(population_fit, key=lambda x: x[0])
                s=math.ceil(0.1*pop_size)

                for k in range(s):
                    new_population.append(sorted_population[k][1])
                    population.remove(sorted_population[k][1])
        ...
        # writer.save()
    fig.update_layout(
    title="Fitness Value Vs Generations Elitism Random population",
